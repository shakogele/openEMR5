...
if ($formid) {
    $sql = "SELECT fo.*, pat.id, CONCAT(pat.fname, " ", pat.lname) as patient FROM form_observation fo LEFT JOIN patient_data pat ON pat.id=fo.pid WHERE fo.id=? AND fo.pid = ? AND fo.encounter = ?";
